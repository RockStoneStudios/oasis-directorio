...
export default function AtmMapaPage() {
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden bg-background">
      ...
      {/* 👑 HEADER MODIFICADO: Aquí metimos el botón directamente */}
      <header className="border-b border-border/50 bg-background px-4 py-3 shrink-0 z-20">
        <div className="container mx-auto flex items-center justify-between">
          
          {/* Bloque Izquierdo: Botón Atrás + Logo/Título */}
          <div className="flex items-center gap-3">
            <button
              onClick={() => router.back()}
              className="p-1.5 rounded-lg text-muted-foreground hover:text-foreground hover:bg-muted transition-colors cursor-pointer active:scale-95"
              title="Volver atrás"
            >
      ...
      {/* 🗺️ MAIN TOTALMENTE LIMPIO: Los filtros ahora flotan solitos sin estorbar */}
        ...
        {!loading && (
          <div className="absolute top-4 inset-x-0 z-10 flex flex-col items-center gap-2 px-4 pointer-events-none">
            ...
            {banksAvailable.length > 0 && (
              <div className="flex gap-2 overflow-x-auto pb-1 max-w-full no-scrollbar pointer-events-auto bg-background/70 backdrop-blur-md p-1.5 rounded-xl shadow-md border border-border/40">
                <button
            ...
            <div className="flex gap-1.5 overflow-x-auto max-w-full no-scrollbar pointer-events-auto bg-background/80 backdrop-blur-md px-2 py-1 rounded-full shadow-sm border border-border/30 text-[11px]">
              <button
