...
function IncendiosMapViewComponent() {
  ...
  // 2. Control de mapa Mapbox con Zoom incrementado
  useEffect(() => {
    ...
    if (!mapRef.current) {
      const map = new mapboxgl.Map({
        ...
        zoom: 9.85, // 👈 Zoom incrementado un ~12% (antes 8.8)
        pitch: 35,  
        bearing: 0,
        antialias: true
      });
  ...
  // 3. Renderizado de Marcadores y Popups
  useEffect(() => {
    ...
    focos.forEach((foco) => {
      ...
      el.addEventListener('click', (e) => {
        ...
        map.easeTo({
          center: [lng, lat],
          zoom: 14.5, // 👈 Zoom de enfoque al hacer clic más cercano (antes 13)
          pitch: 55,
          duration: 600
        });
      });
